...
func (v *ArchitectureComplianceValidator) ValidateArchitecturePrinciples(t *testing.T) {
	...
	switch v.Architecture {
	case "standard":
		validator := NewWebAPIValidator(v.ProjectPath, "standard")
		validator.ValidateArchitecturePrinciples(t, "standard")
	case "clean":
		validator := NewCleanArchitectureValidator(v.ProjectPath)
		validator.ValidateCleanArchitecturePrinciples(t)
	case "ddd":
		validator := NewDDDValidator(v.ProjectPath)
		validator.ValidateDDDPrinciples(t)
	case "hexagonal":
		validator := NewHexagonalValidator(v.ProjectPath)
